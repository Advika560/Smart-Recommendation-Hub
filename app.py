...
if st.button("🚀 Submit Profile"):
    ...
    st.success(f"Selected: {recommendation_type}")
    st.write("DEBUG:", st.session_state.recommendation_type)
# =====================================
# PROFILE COMPLETION
# =====================================
...
st.write(
    ...
    and st.session_state.recommendation_type == "Food"
)
if (
    st.session_state.profile_submitted
    and st.session_state.recommendation_type == "Food"
    ...
    """, unsafe_allow_html=True)
    st.write("FOOD DASHBOARD RUNNING")
    if (
        st.session_state.profile_submitted
        and st.session_state.recommendation_type == "Food"
        and not recommended_foods.empty
    ):
        st.success("FOOD DASHBOARD ACTIVE")
        ...
        favorites_df = pd.DataFrame(
            st.session_state.favorites,
            columns=["Favorite Foods"]
        )
        st.download_button(
            label="📥 Download Favorites CSV",
            data=favorites_df.to_csv(index=False),
            file_name="favorite_foods.csv",
            mime="text/csv"
        )
        if st.session_state.favorites:
            for food in st.session_state.favorites:
                st.markdown(f"• {food}")
        else:
            st.info("No favorite foods saved yet.")
...
if (
    ...
    if (
        st.session_state.profile_submitted
        and st.session_state.recommendation_type == "Products"
    ):
        ...
        st.write("Product recommendation system coming next...")
    ...
    if (
        st.session_state.profile_submitted
        and st.session_state.recommendation_type == "Travel"
    ):

        st.title("✈️ Travel Recommendations")

        st.success(f"Welcome {name}")

        st.write("Travel recommendation system coming next...")